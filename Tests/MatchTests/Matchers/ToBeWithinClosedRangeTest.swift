//
//  ToBeWithinClosedRangeTest.swift
//  Match
//
...
final class ToBeWithinClosedRangeTests: XCTestCase {
    ...
    func testPositiveEvaluation() throws {
        XCTAssertEqual(
            ...
            .passed,
            "ToBeWithinRange() should be evaluated as `passed` when a value is inside a closed range"
        )
        
        XCTAssertEqual(
            makeMatcher(actualValue: 10, range: 0...10).evaluate().evaluationStatus,
            .passed,
            "ToBeWithinRange() should be evaluated as `passed` when a value is an upperbound of a closed range"
        )
        
        XCTAssertEqual(
            makeMatcher(actualValue: 15, range: 0...10).evaluate().evaluationStatus,
            .failed,
            "ToBeWithinRange() should be evaluated as `failed` when a value is not inside a closed range"
        )
    }
    ...
    func testNegatedEvaluation() throws {
        XCTAssertEqual(
            ...
            .failed,
            "Negated ToBeWithinRange() should be evaluated as `failed` when a value is inside a closed range"
        )
        
        XCTAssertEqual(
            makeMatcher(actualValue: 10, range: 0...10, evaluationType: .negated).evaluate().evaluationStatus,
            .failed,
            "Negated ToBeWithinRange() should be evaluated as `failed` when a value is an upperbound of a closed range"
        )
        
        XCTAssertEqual(
            makeMatcher(actualValue: 18, range: 0...10, evaluationType: .negated).evaluate().evaluationStatus,
            .passed,
            "Negated ToBeWithinRange() should be evaluated as `passed` when a value is not inside a closed range"
        )
    }
...
private extension ToBeWithinClosedRangeTests {
    func makeMatcher<T: Comparable>(actualValue: T, range: ClosedRange<T>, evaluationType: EvaluationType = .positive) -> ToBeWithinClosedRange<T> {
        return ToBeWithinClosedRange(
