...
/// expect(2).toBeLessThan(2) // Passes
/// expect(5).toBeLessThan(2) // Fails
/// ```
///
public struct ToBeLessThanOrEqualTo<ResultType: Comparable>: Matcher {
    public let matcherName: String = "toBeLessThanOrEqualTo"
    ...
    public func evaluate() -> EvaluationResult {
        ...
        let isLessThanOrEqual = actualValue <= expectedValue
        
        let evaluationStatus = EvaluationStatus(boolValue: isLessThanOrEqual, evaluationType: expectation.evaluationType)
        let message: String = {
            switch evaluationStatus {
            case .passed:
                return "Received: \(actualValue) is\(isNegated ? " not" : "") less than or equal to: \(expectedValue)"
            case .failed:
                return "Expected the received value: \(actualValue)\(isNegated ? " not" : "") to be less than or equal to: \(expectedValue), but it was\(isNegated ? "" : " not")"
            }
        }()
...
public extension Expectation where ReturnType: Comparable {
    /// Verifies whether the actual value is less than or equal to expected value.
    ///
